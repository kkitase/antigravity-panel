...
### 更改
- **调整配置选项**: 全局将 `gagp` 前缀更名为 `tfa` (Toolkit for Antigravity) 以保持一致性。
- **配置项**: 所有设置项现已使用 `tfa` 前缀（例如 `tfa.quotaDisplayStyle`）。
...
- **视觉自定义**: 新增 `tfa.quotaDisplayStyle` 设置项，允许用户在全新的“半圆弧”与“经典圆环”样式间自由切换。
- **响应式页脚**: 优化了侧边栏页脚在极端窄宽度下的表现，支持按钮自动换行及布局自适应。

### 改进
- **类型安全升级**: 消除了核心 Webview 组件中的 `any` 类型使用，提升系统稳定性。
- **数学精度优化**: 将 SVG 几何计算提取至独立工具库，并配备了专门的几何逻辑单元测试。

### 修复
- **侧边栏布局溢出**: 解决了在侧边栏宽度极窄时，底部反馈与讨论按钮被截断的问题。
