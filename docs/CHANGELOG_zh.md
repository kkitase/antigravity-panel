...
- **绝对重置时间戳**: 在状态栏和仪表盘中新增了 `resetTimestamp` 显示格式选项，允许用户查看配额重置的具体时刻（例如：`02/14 14:30`）。
- **可折叠操作面板**: 在侧边栏页脚增加了折叠/展开功能。用户现在可以点击 "Auto-Accept" 行来收起或展开工具按钮面板，以节省垂直空间。
- **完善多语言支持**: 针对所有新增功能，同步补齐了全部 13 种支持语言的专业翻译。

### 修复

- **Windows 进程检测**: 优化了 Windows 系统下的本地服务发现解析逻辑。通过将端口探测升级为 `Get-NetTCPConnection` 方式，显著提升了 "no_process" 错误诊断的准确性。
- **UI 布局自适应**: 修复了在侧边栏极窄视图下 "Feedback" 和 "Star" 按钮溢出的问题，现在它们可以正常换行。
- **强类型重构**: 对核心 UI 组件（`SidebarApp`, `QuotaPie`, `SidebarFooter`）进行了深度重构，确保在严格编译模式下达到 100% 类型安全。
- **快捷连接修复**: 解决了导致工具栏中“免责声明”（关于按钮）因路径错误无法正常打开的问题。
...
- **探测逻辑**: 增强了语言服务器探测逻辑，提升了复杂环境下的识别成功率与连接稳定性。

### 修复
 
- **多根工作区 LS 检测**: 通过信任直接子进程和兄弟进程的父进程 ID (PPID) 关系，解决了在未保存的多根工作区中语言服务器检测失败的问题。
- **启动稳定性**: 通过移除插件激活期间过早的配额刷新调用，修复了 "server info not available" 报警。
- **配额数据可靠性**: 为 `resetTime` 增加了健壮的 NaN 检查，确保在服务器返回无效日期格式时自动回退到从现在起 24 小时。
 
### 改进
 
- **API 性能**: 将 HTTP 超时增加到 12 秒，防止在本地网络响应较慢时过早失败。
- **配额显示**: 增强了重置计时器，对于超过 24 小时的时长显示天数（例如 `5d 13h`）。
- **默认阈值**: 更新了默认配额警告阈值为 40%，紧急阈值为 20%。
- **配额日志**: 输出现在包含用户邮箱地址，以便更轻松地识别。
